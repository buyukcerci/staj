...
DEFAULT_PROMPT_FILE = PROMPTS_DIR / "distance_estimation_vlm.txt"
...
def normalize_result(parsed: Dict[str, Any]) -> Dict[str, Any]:
    distance_cm = normalize_float(parsed.get("estimated_distance_cm"))
    distance_m = distance_cm / 100.0 if distance_cm is not None else None

    return {
        "target_pair": str(parsed.get("target_pair", "")).strip(),
        "estimated_distance_m": "" if distance_m is None else format_float(distance_m),
        "estimated_distance_cm": "" if distance_cm is None else format_float(distance_cm),
...
def response_text(response_json: Dict[str, Any]) -> str:
    ...
    direct = str(response_json.get("response") or "").strip()
    if direct:
        candidates.append(direct)

    thinking = str(response_json.get("thinking") or "").strip()
    if thinking:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--experiment",
        default="distance_estimation_vlm",
